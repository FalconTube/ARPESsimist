...
class MapParameterBox(QDialog):
    ...
    def init_box(self):
        ...
        if not self.pol_available:
            scan_min = QLineEdit("0.0", self)
            scan_max = QLineEdit("20.0", self)
            # pol_steps = QLineEdit('20.0', self)
            scan_min_label = QLabel("Azimuth Start", self)
            scan_max_label = QLabel("Azimuth End", self)
            # self.pol_steps_label = QLabel('Polar Stepsize', self)

            self.lay.addWidget(scan_min, 10, 1)
            self.lay.addWidget(scan_max, 11, 1)
            # self.lay.addWidget(self.pol_steps, 9, 1)

            self.lay.addWidget(scan_min_label, 10, 0)
            self.lay.addWidget(scan_max_label, 11, 0)
